...
test("AutoFromUsa is rendered correctly", () => {
  render(
   <Provider store={store}> <AutoFromUsa className={mockSectionClassName} heading={mockHeadingText} /></Provider>
  );
});

test("AutoFromUsa contains elements", () => {
  const { getByTestId } = render(
   <Provider store={store}> <AutoFromUsa className={mockSectionClassName} heading={mockHeadingText}>
